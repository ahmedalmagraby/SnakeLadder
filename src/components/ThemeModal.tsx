...
export default function ThemeModal({
  ...
  return (
    <div
      ...
      className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-3 sm:p-4 fade-in"
      onClick={(e) => {
        if (e.target === e.currentTarget) onClose();
      }}
    >
      <div className="panel pop-in w-full max-w-lg p-4 sm:p-6 text-center border-amber-400/40 max-h-[90vh] flex flex-col">
        {/* Header */}
        <div className="flex items-center justify-between pb-3 border-b border-emerald-800/30">
          ...
          <button
            ...
            onClick={onClose}
            aria-label="Close"
            className="w-8 h-8 rounded-lg bg-emerald-950/80 border border-emerald-800/40 text-emerald-300 hover:text-white flex items-center justify-center text-sm font-black cursor-pointer transition-colors"
          >
            ✕
          </button>
        </div>
        ...
        <div className="mt-4 pt-3 border-t border-emerald-800/30 flex justify-end">
          <button
            type="button"
            onClick={onClose}
            className="px-5 py-2 rounded-xl text-xs font-black bg-emerald-900/60 hover:bg-emerald-800/60 border border-emerald-700/50 text-emerald-100 transition-colors cursor-pointer"
          >
            CLOSE
          </button>
        </div>
